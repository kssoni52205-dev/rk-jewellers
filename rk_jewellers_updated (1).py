...
def jobs():
    ...
    if request.method == "POST":
        ...
        try:
            ...
            # Automatic calculation: वजन + नग वजन = कुल वजन; कुल वजन - लॉस = अंतिम वजन
            gross_weight = parse_float("gross_weight")
            nag_weight = parse_float("nag_weight")
            loss = parse_float("loss")
            stone_weight = parse_float("stone_weight")

            total_weight = gross_weight + nag_weight
            final_weight = total_weight - loss
            ...
            if gross_weight < 0:
                raise ValueError("वजन negative nahi ho sakta.")

            if nag_weight < 0:
                raise ValueError("नग वजन negative nahi ho sakta.")

            if loss < 0:
                raise ValueError("लॉस negative nahi ho sakta.")

            if stone_weight < 0:
                raise ValueError("ताँच negative nahi ho sakta.")

            if final_weight < 0:
                raise ValueError("अंतिम वजन negative nahi ho sakta.")
    ...
    if edit_job:
        ...
        form_total = edit_job["total_weight"]
        form_loss = edit_job["loss"]
        form_final = edit_job["final_weight"] or edit_job["net_weight"]
    ...
    body += f"""
            ...
            <div>
                <label>वजन (g)</label>
                <input id="gross_weight" name="gross_weight" type="number" step="0.001" min="0" value="{form_gross}" oninput="calculateWeight()">
            </div>

            <div>
                <label>+नग वजन (g)</label>
                <input id="nag_weight" name="nag_weight" type="number" step="0.001" min="0" value="{form_nag}" oninput="calculateWeight()">
            </div>

            <div>
                <label>=कुल वजन (g) — Automatic</label>
                <input id="total_weight" type="number" step="0.001" value="{form_total}" readonly>
            </div>

            <div>
                <label>-लॉस (g)</label>
                <input id="loss" name="loss" type="number" step="0.001" min="0" value="{form_loss}" oninput="calculateWeight()">
            </div>

            <div>
                <label>=अंतिम वजन (g) — Automatic</label>
                <input id="final_weight" type="number" step="0.001" value="{form_final}" readonly>
            </div>

            <div>
                <label>ताँच (g)</label>
                <input name="stone_weight" type="number" step="0.001" min="0" value="{form_stone}">
            </div>
        ...
        body += f"""
            ...
            <tr>
                ...
                <td>
                    ...
                    वजन: {r["gross_weight"] or 0}g
                    <br>
                    +नग वजन: {r["nag_weight"] or 0}g
                    <br>
                    =कुल वजन: {r["total_weight"] or 0}g
                    <br>
                    -लॉस: {r["loss"] or 0}g
                    <br>
                    =अंतिम वजन: {r["final_weight"] or r["net_weight"] or 0}g
                    <br>
                    ताँच: {r["stone_weight"] or 0}g
                </td>
    ...
    function calculateWeight(){
        const w = parseFloat(document.getElementById('gross_weight')?.value) || 0;
        const n = parseFloat(document.getElementById('nag_weight')?.value) || 0;
        const l = parseFloat(document.getElementById('loss')?.value) || 0;
        const total = w + n;
        const finalW = total - l;
        const totalBox = document.getElementById('total_weight');
        const finalBox = document.getElementById('final_weight');
        if (totalBox) totalBox.value = total.toFixed(3);
        if (finalBox) finalBox.value = finalW.toFixed(3);
    }
    calculateWeight();
    </script>
    """
