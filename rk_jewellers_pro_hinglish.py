...
# PDF labels are kept in Hinglish/English, so no Devanagari font is required.
PDF_FONT = "Helvetica"
...
def setup():
    ...
    # Old fields sirf database compatibility ke liye
    ...
    # Old columns rakhe hain, lekin UI/calculation mein nahi
    add_column(
        "jobs",
        "touch_weight",
        "REAL DEFAULT 0"
...
def jobs():
    ...
    if request.method == "POST":
        ...
        try:
            ...
            # =================================================
            #
            # Vajan + Nag Vajan = Kul Vajan
            # Kul Vajan - Loss = Final Vajan
            # Final Vajan - Stone Vajan = Net Vajan
            #
            ...
            if weight < 0:
                ...
                raise ValueError(
                    "Vajan negative nahi ho sakta."
                )

            if nag_weight < 0:

                raise ValueError(
                    "Nag Vajan negative nahi ho sakta."
                )
            ...
            if stone_weight < 0:
                ...
                raise ValueError(
                    "Stone Vajan negative nahi ho sakta."
                )

            if diamond_weight < 0:

                raise ValueError(
                    "Diamond Weight negative nahi ho sakta."
                )

            if diamond_count < 0:

                raise ValueError(
                    "Diamond Ginti negative nahi ho sakti."
                )

            if quantity < 1:

                raise ValueError(
                    "Nag kam se kam 1 hona chahiye."
                )
...
def ledger_pdf(customer_id):
    ...
    story.append(
        Paragraph(
            "Vajan Details • Diamond Details • Payment Details",
            subtitle_style
        )
    )
    ...
    if not jobs_rows:
        ...
        story.append(
            Paragraph(
                "Koi Job Entry available nahi hai.",
                normal
            )
        )
